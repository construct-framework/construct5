...
<section class="profile-edit<?php echo htmlspecialchars($this->params->get('pageclass_sfx')) ?>">
	...
	<form id="member-profile" action="<?php echo JRoute::_( 'index.php' ); ?>" method="post" name="userform" autocomplete="off" class="user">
		<fieldset>
			<label for="username">
				...
				<?php echo htmlspecialchars($this->user->get('username')); ?>
			</label>
			<label for="name">
				<?php echo JText::_( 'Your Name' ); ?>:
				<input class="inputbox" type="text" id="name" name="name" value="<?php echo htmlspecialchars($this->user->get('name')); ?>" size="40">
			</label>
			<label for="email">
				<?php echo JText::_( 'email' ); ?>:
				<input class="inputbox required validate-email" type="email" id="email" name="email" value="<?php echo htmlspecialchars($this->user->get('email'));?>" size="40">
			</label>
		...
		<input type="hidden" name="username" value="<?php echo htmlspecialchars($this->user->get('username'));?>">
